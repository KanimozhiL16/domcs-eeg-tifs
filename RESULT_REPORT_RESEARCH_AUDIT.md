...
Audit status: updated for the submitted IEEE TIFS reproducibility package.
...
The repository separates three things that should not be confused:

| Category | File | Purpose |
|---|---|---|
| Submitted manuscript table | `experiments/results/TABLE_III_main_results.csv` | Final submitted Table III values as written in the paper |
...
| Source | Mean EER (%) | Std EER (%) | Mean AUC | Std AUC | CRR (%) | Interpretation |
|---|---:|---:|---:|---:|---:|---|
| Submitted paper Table III | 3.76 | 0.21 | 0.9931 | 0.0007 | 86.95 +/- 0.40 | Paper-locked table |
| Original NVIDIA A100 run | 3.76 | 0.21 | 0.9931 | 0.0007 | not recomputed in raw CSV | Raw implementation evidence |
| Independent NVIDIA A100 rerun | 3.82 | 0.29 | 0.9929 | 0.0011 | not recomputed in raw CSV | Reproducibility check |

The raw rerun is close to the submitted result but not bit-identical. This is expected for GPU training unless all nondeterministic CUDA behavior is disabled. The ethical claim is statistical reproducibility within tolerance, not exact numeric equality for every rerun.
...
| Submitted Table III paper lock | Present |
...
```text
OK: submitted paper table, config, original raw evidence, and rerun evidence are consistent within tolerance.
```
...
3. Single-seed or 200-epoch diagnostic runs should not replace the 5-seed 60-epoch submitted Table III result.
4. Older local folders may contain draft values and should not be used as the canonical submitted paper source.

## Final Verdict

The repo is organized around a reviewer-safe position:

- the submitted manuscript table is preserved,
...
This is the ethical way to align the paper and implementation: preserve the submitted paper numbers, preserve raw evidence, and avoid claiming impossible bitwise equality for nondeterministic GPU training.
